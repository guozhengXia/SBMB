package com.honor.sbmb.front.filter;
...
public class LoginFilter implements Filter {
    ...
    private String[] IGNORE_URI = {
            "/front/user/register.do",//注册接口
            "/front/user/login.do",//登录接口
            "/front/test.do",//测试接口
    };
    @Override
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(contain(uri)){//如果包含在忽略uri中则不做处理
            ...
            if (token != null && token.startsWith("Bearer "+TokenServiceImpl.TOKEN_TYPE_FRONT)) {//如果token存在，且是front端的token
                boolean exists = tokenService.exists(token);
                if(exists){//如果token存在则放行
                    //重置token的有效期
                    tokenService.resetExpireTime(token, TokenServiceImpl.TOKEN_OUT_TIME_FRONT);
                    filterChain.doFilter(servletRequest,servletResponse);
                }else {//token不存在或失效
